...
function loadEnvFromFile() {
  ...
  try {
    const contents = fsSync.readFileSync(envFile, 'utf-8');
    ...
    if (error.code !== 'ENOENT') {
      console.warn(`[#ENV] Falha ao ler ${envFile}: ${error.message}`);
    }
  }
}
...
try {
  ...
  if (error.code !== 'MODULE_NOT_FOUND') {
    console.warn(`[#ENV] Erro ao carregar dotenv: ${error.message}`);
  }
  loadEnvFromFile();
}
